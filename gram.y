...
expr(A) ::= expr    PLUS   expr .  { std::cout << "\tadding two expressions" << std::endl;         A = int_to_value(2) + int_to_value(2); }
...
expr(A) ::= expr(B) MINUS  expr(C) .  { std::cout << "\tsubtracting two expressions" << std::endl; A = B + C; }
expr(A) ::= expr(B) TIMES  expr(C) .  { std::cout << "\tmultiplying two expressions" << std::endl; A = B + C; }
expr(A) ::= expr(B) DIVIDE expr(C) .  { std::cout << "\tdividing two expressions" << std::endl;    A = B + C; }
